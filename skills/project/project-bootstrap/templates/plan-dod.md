...
6. **Docs updated inline** — if this task changes user-facing behavior (setup, scripts, env vars, features) update `README.md` in the same task. If it changes architecture, file layout, data flow, or conventions, update `CLAUDE.md` / `AGENTS.md` in the same task. State "no doc impact" explicitly in the task report when none apply. Doc edits count toward DoD and must pass `npm run verify`.
...
The `SubagentStop` hook in `.claude/settings.json` re-runs `npm run verify` automatically when a subagent finishes. A subagent that reports done with failing checks will be blocked.
...
- Update `CLAUDE.md` / `AGENTS.md` for any architectural changes (new modules, data flow, conventions, file layout, build steps).
