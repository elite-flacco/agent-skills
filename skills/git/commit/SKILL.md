...
- **Emoji**: Each commit type pairs with an appropriate emoji (e.g. ✨ `feat`, 🐛 `fix`, 📝 `docs`, ♻️ `refactor`, 🔧 `chore`). The full list of prefixes is in `references/emoji-commits.md` — load it when you need to pick the right emoji for an unusual change.
